...
if(displayPanel){
  const section=document.createElement('section');section.className='date-time-format-setting';section.innerHTML='<div class="date-time-format-controls"><label>Date format <span class="mobile-setting-description">Date style</span><select data-date-format><option>D MMM YYYY</option><option>DD/MM/YYYY</option><option>MM/DD/YYYY</option><option>YYYY-MM-DD</option></select></label><label>Time format <span class="mobile-setting-description">Clock style</span><select data-time-format><option value="false">24-hour</option><option value="true">12-hour</option></select></label></div><p>Used consistently throughout the calendar and date/time fields.</p>';displayPanel.prepend(section);
