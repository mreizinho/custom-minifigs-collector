...
if(displayPanel){
  const section=document.createElement('section');section.className='date-time-format-setting';section.innerHTML='<div class="date-time-mobile-headings" aria-hidden="true"><strong>Date and time format</strong></div><div class="date-time-format-controls"><label><span class="desktop-format-title">Date format</span><select aria-label="Date format" data-date-format><option>D MMM YYYY</option><option>DD/MM/YYYY</option><option>MM/DD/YYYY</option><option>YYYY-MM-DD</option></select></label><label><span class="desktop-format-title">Time format</span><select aria-label="Time format" data-time-format><option value="false">24-hour</option><option value="true">12-hour</option></select></label></div><p>Used consistently throughout the calendar and date/time fields.</p>';displayPanel.prepend(section);
